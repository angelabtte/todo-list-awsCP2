            ...
            steps {
                ...
                withCredentials([usernamePassword(credentialsId: 'tokencp3', usernameVariable: 'angelabtte', passwordVariable: 'tokencp3')]) {
                    sh '''
                        ...
                        git stash || echo " Nada que stashear"
                        git pull --rebase origin master || echo "No se pudo hacer pull --rebase"
                        git push origin master || echo "Push rechazado. Requiere intervención manual."
                    '''
                }
            }
        }
